...
if (window.electronAPI) {
    versionTag.onclick = () => {
        showToast('🔄 正在连接星轨... 搜寻新版本中', true);
        window.electronAPI.checkForUpdates();
    };
    window.electronAPI.onUpdateNotAvailable(() => { showToast('✨ 当前已是宇宙最新版本，无需升级'); });
    window.electronAPI.onUpdateError(() => { showToast('⚠️ 检查受阻，请检查网络环境后重试'); });
    window.electronAPI.onUpdateDownloaded((info) => {
        document.getElementById('toastNotification').classList.remove('show');
        updateModalMessage.innerText = `MusicZ 最新代号 ${info.version || ''} 已经准备就绪！\n是否立即重启跃迁？`;
        updateModal.style.display = 'flex'; updateModal.offsetHeight; updateModal.classList.add('show');
    });
    updateCancelBtn.onclick = () => {
        updateModal.classList.remove('show');
        setTimeout(() => { updateModal.style.display = 'none'; }, 300);
    };
    updateOkBtn.onclick = () => { updateOkBtn.innerText = '正在升华...'; window.electronAPI.quitAndInstall(); };
} else {
...
async function playSong(index, resumeTime = 0, forcePlay = true) {
    ...
    try {
        ...
        if (urlData.data && urlData.data[0] && urlData.data[0].url) {
            ...
            if (resumeTime > 0) {
                showToast(actualLevel !== preferredQuality ? `🎵 版权受限，已平滑降级至【${actualQualityDisplay(actualLevel)}】` : `🎵 已切换至【${actualQualityDisplay(actualLevel)}】`);
            }
...
audioPlayer.onplay = () => { 
    ...
    playPauseBtn.classList.add('is-playing'); // 触发 CSS 居中对齐
    
    if(currentView === 'tabBlindBox') {
        const icon = document.querySelector('.blind-box-icon');
...
audioPlayer.onpause = () => { 
    ...
    playPauseBtn.classList.remove('is-playing'); // 取消 CSS 居中对齐
...
function renderBlindBoxUI() {
    ...
    if (isBlindBoxMode && playingList.length > 0) {
        ...
        containerHTML = `<div class="blind-box-container"><div class="blind-box-icon ${iconClass}">🎧</div><div id="slotMachineText" class="slot-text">【 ${currentBlindBoxTag} 】</div><div class="blind-box-title" style="color:#333;">异次元电波已接通</div><div class="blind-box-desc">不要看歌单，闭上眼睛，享受未知的音乐之旅。<br>遇到不喜欢的，直接使用快捷键或下一首按钮。</div><button id="rollBtn" class="blind-box-btn">摇一摇，抽取新曲风</button></div>`;
    } else {
        containerHTML = `<div class="blind-box-container"><div class="blind-box-icon">🎲</div><div id="slotMachineText" class="slot-text" style="color:#ec4141; margin-bottom: 5px;">- 未知惊喜 -</div><div class="blind-box-title">未知的旋律，一期一会</div><div class="blind-box-desc">抛下歌单，交给命运。<br>摇动专属骰子，抽取一段跨越时空的音乐电波。</div><button id="rollBtn" class="blind-box-btn">摇一摇，抽取今日曲风</button></div>`;
    }
    dataList.innerHTML = containerHTML;
    const rollBtn = document.getElementById('rollBtn');
    if (cdRemaining > 0) { rollBtn.classList.add('disabled'); rollBtn.innerText = `重新装填磁带 (${cdRemaining}s)`; }
...
async function handleBlindBoxRoll() {
    ...
    const icon = document.querySelector('.blind-box-icon');
    const title = document.querySelector('.blind-box-title');
    if(title) title.innerText = '正在调配惊喜...';
    if(icon) { icon.innerText = '🎲'; icon.classList.remove('icon-playing'); icon.classList.add('icon-spinning'); }
    ...
    const rollBtn = document.getElementById('rollBtn');
    cdRemaining = 10; rollBtn.classList.add('disabled');
    cdTimer = setInterval(() => {
        cdRemaining--; const btn = document.getElementById('rollBtn'); if(btn) btn.innerText = `重新装填磁带 (${cdRemaining}s)`;
        if (cdRemaining <= 0) { clearInterval(cdTimer); if(btn) { btn.innerText = '摇一摇，抽取新曲风'; btn.classList.remove('disabled'); } }
    }, 1000);
    let slotTimer = setInterval(() => { const st = document.getElementById('slotMachineText'); if(st) st.innerText = blindBoxTags[Math.floor(Math.random() * blindBoxTags.length)]; }, 70);
    setTimeout(async () => {
        clearInterval(slotTimer); const finalTag = blindBoxTags[Math.floor(Math.random() * blindBoxTags.length)]; currentBlindBoxTag = finalTag;
        const st = document.getElementById('slotMachineText');
        if(st) { st.innerText = `【 ${finalTag} 】`; st.style.transform = 'scale(1.2)'; setTimeout(() => { if(document.getElementById('slotMachineText')) document.getElementById('slotMachineText').style.transform = 'scale(1)'; }, 300); }
        try {
            ...
            } else { if(document.querySelector('.blind-box-title')) document.querySelector('.blind-box-title').innerText = '电波干扰，没有找到该标签歌曲'; if(icon) icon.classList.remove('icon-spinning'); }
        } catch(e) { if(document.querySelector('.blind-box-title')) document.querySelector('.blind-box-title').innerText = '网络断开，请重试'; if(icon) icon.classList.remove('icon-spinning'); }
        isSlotting = false;
    }, 1200);
}
